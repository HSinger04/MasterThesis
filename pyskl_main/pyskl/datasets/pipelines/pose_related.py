...
@PIPELINES.register_module()
class RandomRot:
    # TODO: Same as tools.random_rot from STTFormer
...
class PreNormalize3D:
    ...
    def __call__(self, results):
        ...
        # TODO: Might be equal to seq_translation from seq_transformation.py
        if self.align_center:
            if skeleton.shape[2] == 25:
                # Take the second joint as origin
                main_body_center = skeleton[0, 0, 1].copy()
            else:
                main_body_center = skeleton[0, 0, -1].copy()
            # TODO: mask is a boolean array in almost the same shape of skeleton
            #  i.e. skeleton's shape: (num_bodies, num_frames, 25, 3), mask: (num_bodies, num_frames, 25, 1).
            # I assume it has false whereever we have an empty frame and there, they multiple with 0.
...
@PIPELINES.register_module()
class GenSkeFeat:
    # TODO: Just transforming to the modality
