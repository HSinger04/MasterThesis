...
class UniformSampleDecode:
    ...
    def _get_clips(self, full_kp, clip_len):
        # For dgstgcn: Number of bodies (1 or 2), number of frames, number of joints (25), xyz (3)
        M, T, V, C = full_kp.shape
        clips = []

        for clip_idx in range(self.num_clips):
            pi = self.p_interval
            # Ratio is always 1 for dgstgcn
            ratio = np.random.rand() * (pi[1] - pi[0]) + pi[0]
            ...
            if num_frames < clip_len:
                start = np.random.randint(0, num_frames)
                # ndarray of length self.clip_len, starting at "start" and then just cycling % num_frames
                inds = (np.arange(start, start + clip_len) % num_frames) + off
                clip = full_kp[:, inds].copy()
            elif clip_len <= num_frames < 2 * clip_len:
                # Summary: At the inds, skip a frame.
                basic = np.arange(clip_len)
                # Take num_frames - clip_len many indices randomly
                inds = np.random.choice(clip_len + 1, num_frames - clip_len, replace=False)
                offset = np.zeros(clip_len + 1, dtype=np.int64)
                offset[inds] = 1
                # e.g. if offset = [0 0 1 0 1 1], then np.cumsum(offset)[:-1] == [0 0 1 1 2]
                inds = basic + np.cumsum(offset)[:-1] + off
                clip = full_kp[:, inds].copy()
            else:
                # Summary: First, split the frames into an almost uniform interval (bids). Then, pick a random
                # member of the sub-intervals defined through bids (bst + offset, where offset determines
                # the range of the sub-intervals).
                # e.g. num_frames == 325, clip_len == 100:
                # bids == [0, 3, 6, 9, 13, 16, 22, 26, ...]
                bids = np.array([i * num_frames // clip_len for i in range(clip_len + 1)])
                # bsize = [3 3 3 4 3 6 4 ...]
                bsize = np.diff(bids)
                bst = bids[:clip_len]
                # random array of length clip_len (e.g. 100). [0 2 1 3 2 5 2 ...]
                offset = np.random.randint(bsize)
                inds = bst + offset + off
                clip = full_kp[:, inds].copy()
            clips.append(clip)
    ...
    def _handle_dict(self, results):
        # this method gets used for NTU
        assert 'keypoint' in results
        kp = results.pop('keypoint')
        # This part gets skipped for NTU
    ...
    def __call__(self, results):
        """
        UniformSampleDecode, because: While we randomly pick frames, we do so while still making sure that we do so with
        more or less consistent gaps.
