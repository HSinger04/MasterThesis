...
def parse_skeleton_file(ske_name, root='nturgb+d_skeletons'):
    ...
    for k in body_data:
        # kpt seems to be the joints. Compare with motion calculation in get_raw_denoised_data.py
...
def non_zero(kpt):
    """ TODO: Looks like it trims the almost-zeros at the beginning and the end?
...
def gen_keypoint_array(body_data):
    ...
    if len(body_data) == 1:
        ...
        # Keypoints = joints, as 2 * 25 * 3 = 2 * 75 = 150
        keypoint = np.zeros((2, max_fidx, 25, 3), np.float32)

        # start and ends, see get_raw_denoised_data.py
        ...
        # TODO: No idea what is done here
